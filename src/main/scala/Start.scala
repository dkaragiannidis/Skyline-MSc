...
//    var efNumberRecord=50
    ...
    var eliminationFilterSelect=SfsData.sqlContext.sql(s"select *  from xxx order by Entropy asc  limit $efNumberRecord")
    ...
    SfsData=SfsData.repartition(partition).filter(x=>cleanData(x))
    SfsData.show()
  ...
  println("ksekinaei o lessssssssssssssssssssssssssssssssssssssssssssssssssssssssss")
